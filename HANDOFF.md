...
- Map is `maps-compose`'s `GoogleMap` with custom `BitmapDescriptorFactory` markers; clinic search is the Google Places API (`places:searchText`, query "dermatology clinic", 15km location bias, raw `HttpURLConnection` call rather than the Places SDK — see the header-attachment comment in `ClinicLocatorScreen.kt`, it needs `X-Android-Package`/`X-Android-Cert` manually or Google blocks it). Driving routes and ETAs come from Google's Routes API (`computeRoutes`, same key and headers); OSRM is no longer used.
- **"Still open?" votes need Firestore rules published by hand.** The clinic popup's crowd-sourced open/closed tally (`ClinicVotes.kt`) writes to Firestore directly from the app. The rules live in `firestore.rules` at the repo root and are not deployed automatically — paste the file into Firebase Console → Firestore Database → Rules → Publish. Without them, Firestore either rejects every vote (production-mode default) or lets anyone overwrite the counts (test-mode default). The rules only allow a count change that exactly matches the caller's own vote change in the same transaction, and they deny all other Firestore paths, since nothing else in the app uses Firestore.
...
- When Places doesn't return `regularOpeningHours.openNow`, the clinic shows a grey "Hours unknown" badge instead of guessing "Open Now" (`Clinic.openNow` is nullable for this).
