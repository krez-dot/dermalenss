...
**Live-verified:** Register (real Firebase account + verification email + local profile), Login (Firebase auth + resolves matching local profile), Logout (correctly signs out of Firebase too), Change Password (reauthenticate + update, confirmed via a live "Saved!" state), Verify Your Email (an unverified account is correctly blocked, Resend Email works), Delete Account (reauthenticates, deletes local scan records + photos, deletes the Firebase account), Forgot Password (full real-inbox round trip confirmed 2026-09-18: request reset → email arrives → reset link → log in with new password).

**Password policy is enforced server-side, not just client-side.** The app's own Register/Change Password screens validate 8+ chars with upper/lower/number/special locally, but that check has no effect on Firebase's hosted password-reset page (reached via the "Forgot password?" email link) — that page is outside the app entirely. Live-testing on 2026-09-18 confirmed this gap: the reset page accepted a plain 6-character password with no complexity at all. Fixed via **Firebase Console → Authentication → Settings → Password policy**: minimum length 8, all four complexity requirements checked, enforcement mode set to **Require** (not the default "Notify," which only warns and still lets weak passwords through). Re-verified live: the reset page now rejects non-compliant passwords with a specific "Missing password requirements" message. This policy applies to every Firebase Auth password path project-wide, not just the reset page.
